...
def parse_hour(string: str, now=datetime.utcnow()) -> datetime:
    ...
    if match is None:
        ...
        date = datetime(year=now.year, month=now.month, day=now.day, hour=hours, minute=minutes)
...
assert parse_hour("9H30", now=datetime(year=1984, month=1, day=10, hour=6)) == datetime(year=1984, month=1, day=10, hour=9, minute=30)
assert parse_hour("8h", now=datetime(year=1984, month=1, day=10, hour=18)) == datetime(year=1984, month=1, day=11, hour=8, minute=00)
...
def message(list: list, string: str):
